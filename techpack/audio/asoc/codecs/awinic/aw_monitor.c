...
static int aw_monitor_get_voltage(struct aw_device *aw_dev, unsigned int *vol)
{
	...
	aw_dev_dbg(aw_dev->dev, "chip voltage is %d", *vol);
...
static int aw_monitor_get_temperature(struct aw_device *aw_dev, int *temp)
{
	...
	if (desc->reg == AW_REG_NONE) {
		...
		*temp = (*temp) / 10;
		...
		aw_dev_dbg(aw_dev->dev, "reg val is 0x%04x", reg_val);
		local_temp = reg_val;
	...
	aw_dev_dbg(aw_dev->dev, "chip temperature = %d", *temp);
	return 0;
}
...
static void aw_monitor_parse_hdr(struct aw_device *aw_dev, uint8_t *data)
{
	...
	aw_dev_dbg(aw_dev->dev, "chip name:%s",
		monitor_hdr->chip_type);
	aw_dev_dbg(aw_dev->dev, "ui ver:0x%x",
		monitor_hdr->ui_ver);

	aw_dev_dbg(aw_dev->dev, "monitor_switch:%d, monitor_time:%d (ms), monitor_count:%d",
		monitor_cfg->monitor_switch, monitor_cfg->monitor_time,
		monitor_cfg->monitor_count);

	aw_dev_dbg(aw_dev->dev, "ipeak_switch:%d, gain_switch:%d, vmax_switch:%d",
		monitor_cfg->ipeak_switch, monitor_cfg->gain_switch,
		monitor_cfg->vmax_switch);

	aw_dev_dbg(aw_dev->dev, "temp_switch:%d, temp_aplha:%d, vol_switch:%d, vol_aplha:%d",
