...
static int aw_cali_write_cali_re_to_file(int32_t cali_re, int channel)
{
	...
	if (start_cali == true){
		...
		pr_info("%s: channel:%d buf:%s cali_re:%d start_cali \n",
...
static void aw_cali_parse_dt(struct aw_device *aw_dev)
{
	...
	//aw_dev_info(aw_dev->dev, "cali check :%s",(desc->cali_check_st) ? "enable" : "disable");
...
int aw_cali_parse_re_dt(struct aw_device *aw_dev)
{
	...
	//aw_dev_info(aw_dev->dev, "re min: %d, re max: %d",aw_dev->re_min, aw_dev->re_max);
	return 0;
}
