...
static int parse_chipid(struct aw_haptic *aw_haptic)
{
	...
	for (cnt = 0; cnt < AW_READ_CHIPID_RETRIES; cnt++) {
		...
		if (ret < 0) {
			...
			//aw_info("try to replace i2c addr [(0x%02X)] to read chip id again",aw_haptic->i2c->addr);
...
static int judge_within_cali_range(struct aw_haptic *aw_haptic)
{
	...
	//aw_info("f0_pre = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d",aw_haptic->info.f0_pre, f0_cali_min, f0_cali_max,aw_haptic->f0);
...
static int haptic_playback(struct input_dev *dev, int effect_id, int val)
{
	...
	//aw_info("enter, val=%d, aw_haptic->effect_id=%d, activate_mode=%d",val, aw_haptic->effect_id, aw_haptic->activate_mode);
