...
#define GOODIX_CMD_SET_CONFIG_RETRY	20

#define GOODIX_CMD_COORD			0x00
#define GOODIX_CMD_RAWDATA			0x02
#define GOODIX_CMD_SET_CONFIG_START	0x04
#define GOODIX_CMD_SET_CONFIG_WRITE	0x05
#define GOODIX_CMD_SET_CONFIG_END	0x06
#define GOODIX_CMD_GET_CONFIG_START	0x07
#define GOODIX_CMD_GET_CONFIG_END	0x08
#define GOODIX_CMD_GAME				0x17
#define GOODIX_CMD_NORMAL			0x18
#define GOODIX_CMD_HDLE_MODE		0x29
#define GOODIX_CMD_SLEEP			0x84
#define GOODIX_CMD_PALM				0x70
#define GOODIX_CMD_CAP_DATA_BRD		0x90
#define GOODIX_CMD_GESTURE_START	0xA6
#define GOODIX_CMD_GESTURE_END		0xA7
#define GOODIX_CMD_CHARGER			0xAF

#define GOODIX_CMD_ACK_INVALID          0x00
#define GOODIX_CMD_ACK_IDLE             0x01
...
#define GOODIX_EVENT_TOUCH			0x80
#define GOODIX_EVENT_REQUEST		0x40
#define GOODIX_EVENT_GESTURE		0x20

#define GOODIX_POINT_TYPE_STYLUS_HOVER	0x01
#define GOODIX_POINT_TYPE_STYLUS		0x03

enum goodix_brl_request_code {
...
static int goodix_brl_select_spi_mode_bra(struct goodix_ts_core *cd)
{
	int ret;
	int i;
...
static int goodix_brl_dev_confirm(struct goodix_ts_core *cd)
{
...
static int goodix_brl_after_reset_bra(struct goodix_ts_core *cd)
{
	u8 reg_val[2] = {0};
	u8 temp_buf[12] = {0};
	int ret;
	...
	/* select spi mode */
	ret = goodix_brl_select_spi_mode_bra(cd);
	if (ret < 0)
		return ret;
...
static int goodix_brl_power(struct goodix_ts_core *cd, bool on)
{
	int ret = 0;
	...
	if (on) {
		...
		ret = goodix_brl_dev_confirm(cd);
		if (ret < 0)
			goto power_off;

		if (cd->bus->ic_type == CHIP_TYPE_BRA)
		{
			ret = goodix_brl_after_reset_bra(cd);
			if (ret < 0)
				goto power_off;
		}
...
#if 0
int goodix_brl_sleep(struct goodix_ts_core *cd)
{
	struct goodix_ts_cmd sleep_cmd;

	sleep_cmd.cmd = GOODIX_CMD_SLEEP;
	sleep_cmd.len = CONFIG_CMD_LEN;
...
int goodix_brl_gesture(struct goodix_ts_core *cd, bool on)
{
	struct goodix_ts_cmd cmd;

	if (on)
		cmd.cmd = GOODIX_CMD_GESTURE_START;
	else
		cmd.cmd = GOODIX_CMD_GESTURE_END;
	cmd.len = CONFIG_CMD_LEN + 1;
	cmd.data[0] = 0;
...
static int goodix_brl_reset(struct goodix_ts_core *cd)
{
	u8 reg_val = GOODIX_RESET_EN;
	int ret;
...
static int goodix_brl_irq_enable(struct goodix_ts_core *cd, bool enable)
{
	...
	ts_err("warnning: irq deepth inbalance: %d!", enable);

	return 0;
}

static int goodix_brl_read(struct goodix_ts_core *cd, unsigned int addr,
...
static int goodix_brl_write(struct goodix_ts_core *cd, unsigned int addr,
		 unsigned char *data, unsigned int len)
{
	struct goodix_bus_interface *bus = cd->bus;
...
static int goodix_brl_send_cmd(struct goodix_ts_core *cd,
	struct goodix_ts_cmd *cmd)
{
	int ret, retry, i;
	...
	while (retry++ < GOODIX_CMD_RETRY) {
		...
		for (i = 0; i < GOODIX_CMD_RETRY; i++) {
			...
			if (cmd_ack.ack == GOODIX_CMD_ACK_OK) {
				msleep(40); // wait for cmd response
				return 0;
			}
			if (cmd_ack.ack == GOODIX_CMD_ACK_BUSY ||
			    cmd_ack.ack == GOODIX_CMD_ACK_INVALID) {
				usleep_range(1000, 1100);
				continue;
			}
			if (cmd_ack.ack == GOODIX_CMD_ACK_BUFFER_OVERFLOW)
	...
	struct goodix_ts_cmd *cfg_cmd)
{
	...
	ret = wait_cmd_status(cd, GOODIX_CMD_ACK_OK,
		GOODIX_CMD_SET_CONFIG_RETRY);
	if (ret) {
...
static int goodix_brl_send_config(struct goodix_ts_core *cd, u8 *cfg, int len)
{
	int ret;
	u8 *tmp_buf;
	...
	cfg_cmd.cmd = GOODIX_CMD_SET_CONFIG_START;
	...
	cfg_cmd.cmd = GOODIX_CMD_SET_CONFIG_WRITE;
	cfg_cmd.len = CONFIG_CMD_LEN;
	ret = send_cfg_cmd(cd, &cfg_cmd);
	if (ret)
...
exit:
	memset(cfg_cmd.buf, 0, sizeof(cfg_cmd));
	cfg_cmd.cmd = GOODIX_CMD_SET_CONFIG_END;
	cfg_cmd.len = CONFIG_CMD_LEN;
 ...
 **/
static int goodix_brl_read_config(struct goodix_ts_core *cd, u8 *cfg, int size)
{
	int ret;
	struct goodix_ts_cmd cfg_cmd;
	struct goodix_ic_info_misc *misc = &cd->ic_info.misc;
	...
	cfg_cmd.cmd = GOODIX_CMD_GET_CONFIG_START;
	cfg_cmd.len = CONFIG_CMD_LEN;
	ret = send_cfg_cmd(cd, &cfg_cmd);
	if (ret) {
...
exit:
	memset(cfg_cmd.buf, 0, sizeof(cfg_cmd));
	cfg_cmd.cmd = GOODIX_CMD_GET_CONFIG_END;
	cfg_cmd.len = CONFIG_CMD_LEN;
	if (send_cfg_cmd(cd, &cfg_cmd)) {
 ...
 */
static int goodix_brl_read_version(struct goodix_ts_core *cd,
...
static int goodix_brl_get_ic_info(struct goodix_ts_core *cd,
...
static int goodix_brl_esd_check(struct goodix_ts_core *cd)
{
	int ret;
	u32 esd_addr;
	u8 esd_value;
...
static int goodix_brl_after_event_handler(struct goodix_ts_core *cd)
{
	struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
	struct goodix_ic_info_misc *misc = &cd->ic_info.misc;
	u8 sync_clean = 0;
				...
				u8 *pre_buf, u32 pre_buf_len)
{
	...
	/* read done */
	goodix_brl_after_event_handler(cd);

	if (touch_num > 0) {
		point_type = buffer[IRQ_EVENT_HEAD_LEN] & 0x0F;
		if (point_type == GOODIX_POINT_TYPE_STYLUS ||
				point_type == GOODIX_POINT_TYPE_STYLUS_HOVER) {
	...
	if (touch_num > 0 && (point_type == GOODIX_POINT_TYPE_STYLUS
				|| point_type == GOODIX_POINT_TYPE_STYLUS_HOVER)) {
		/* stylus info */
...
static int goodix_brl_event_handler(struct goodix_ts_core *cd,
			 struct goodix_ts_event *ts_event)
{
	struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
	struct goodix_ic_info_misc *misc = &cd->ic_info.misc;
	...
	if (event_status & GOODIX_EVENT_TOUCH)
		return goodix_touch_handler(cd, ts_event,
					    pre_buf, pre_read_len);

	if (event_status & GOODIX_EVENT_REQUEST) {
	...
	if (event_status & GOODIX_EVENT_GESTURE) {
...
static int goodix_brl_get_cap_data(struct goodix_ts_core *cd,
				   struct ts_rawdata_info *info)
{
	...
	ret = cd->hw_ops->read(cd, raw_addr, (u8 *)&info->buff[info->used_size],
			       size * sizeof(s16));
	if (ret < 0) {
	...
	ret = cd->hw_ops->read(cd, diff_addr,
			       (u8 *)&info->buff[info->used_size],
			       size * sizeof(s16));
	if (ret < 0) {
...
static int goodix_brl_get_framedata_brd(struct goodix_ts_core *cd,
					     struct ts_rawdata_info *info)
{
	int ret;
	unsigned char val;
	...
	ret = cd->hw_ops->read(cd, flag_addr, frame_buf,
...
static int goodix_brl_get_cap_data_brd(struct goodix_ts_core *cd,
				       struct ts_rawdata_info *info)
{
	struct goodix_ts_cmd temp_cmd;
	int tx = cd->ic_info.parm.drv_num;
	...
	temp_cmd.cmd = GOODIX_CMD_CAP_DATA_BRD;
	temp_cmd.data[0] = 0x81;
	temp_cmd.len = CONFIG_CMD_LEN + 1;
	ret = cd->hw_ops->send_cmd(cd, &temp_cmd);
	if (ret < 0) {
	...
	ret = goodix_brl_get_framedata_brd(cd, info);
	if (ret < 0) {
...
static int goodix_brl_get_capacitance_data(struct goodix_ts_core *cd,
		struct ts_rawdata_info *info)
{
	int ret;
	...
	if (cd->bus->ic_type != CHIP_TYPE_BRD)
		ret = goodix_brl_get_cap_data(cd, info);
	else
		ret = goodix_brl_get_cap_data_brd(cd, info);
...
static struct goodix_ts_hw_ops goodix_brl_hw_ops = {
	.power= goodix_brl_power,
	.gesture = goodix_brl_gesture,
	.reset = goodix_brl_reset,
	.irq_enable = goodix_brl_irq_enable,
	.read = goodix_brl_read,
	.write = goodix_brl_write,
	.send_cmd = goodix_brl_send_cmd,
	.send_config = goodix_brl_send_config,
	.read_config = goodix_brl_read_config,
	.read_version = goodix_brl_read_version,
	.get_ic_info = goodix_brl_get_ic_info,
	.esd_check = goodix_brl_esd_check,
	.event_handler = goodix_brl_event_handler,
	.get_capacitance_data = goodix_brl_get_capacitance_data,
};

struct goodix_ts_hw_ops *goodix_get_hw_ops(void)
{
	return &goodix_brl_hw_ops;
}
