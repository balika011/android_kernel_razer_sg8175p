...
static int fts_input_report_b(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            input_report_abs(input_dev, ABS_MT_POSITION_X, FTS_Y_MAX_DISPLAY_DEFAULT - 1 - events[i].x);
            input_report_abs(input_dev, ABS_MT_POSITION_Y, FTS_X_MAX_DISPLAY_DEFAULT - 1 - events[i].y);
